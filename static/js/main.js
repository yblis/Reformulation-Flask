document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.reuse-history').forEach(button => {
        button.addEventListener('click', function() {
            // Get the data from history item
            const historyItem = this.closest('.accordion-item');
            const contextElement = historyItem.querySelector('.context-text');
            const originalElement = historyItem.querySelector('.original-text');
            const toneElement = historyItem.querySelector('.tone-badge');
            const formatElement = historyItem.querySelector('.format-badge');
            const lengthElement = historyItem.querySelector('.length-badge');
            ...
            if (contextText && contextElement) {
                contextText.value = contextElement.textContent;
                updateTextStats(contextText.value, 'contextCharCount', 'contextWordCount', 'contextParaCount');
            }
            
            if (inputText && originalElement) {
                inputText.value = originalElement.textContent;
                updateTextStats(inputText.value, 'inputCharCount', 'inputWordCount', 'inputParaCount');
            }

            // Set the options
            if (toneElement) {
                const toneValue = toneElement.textContent;
                document.querySelectorAll('#toneGroup .btn').forEach(btn => {
                    if (btn.dataset.value === toneValue) {
                        btn.click();
                    }
                });
            }
            ...
            if (formatElement) {
                const formatValue = formatElement.textContent;
                document.querySelectorAll('#formatGroup .btn').forEach(btn => {
                    if (btn.dataset.value === formatValue) {
                        btn.click();
                    }
                });
            }
            ...
            if (lengthElement) {
                const lengthValue = lengthElement.textContent;
                document.querySelectorAll('#lengthGroup .btn').forEach(btn => {
                    if (btn.dataset.value === lengthValue) {
                        btn.click();
                    }
                });
            }
