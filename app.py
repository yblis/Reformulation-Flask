...
def get_openai_models():
    try:
        ...
        try:
            ...
            error_message = str(e)
            print(f"OpenAI API error: {error_message}")  # For debugging
            if "401" in error_message:
                return jsonify({"error": "Invalid OpenAI API key"}), 401
            return jsonify({"error": f"API request failed: {error_message}"}), 500
            
    except Exception as e:
        print(f"Error in get_openai_models: {str(e)}")  # For debugging
...
def get_anthropic_models():
    try:
        ...
        print(f"Error in get_anthropic_models: {str(e)}")  # For debugging
        return jsonify({"error": str(e)}), 500
...
def get_groq_models():
    try:
        ...
        try:
            ...
            print(f"Groq API error: {str(e)}")  # For debugging
            if "401" in str(e):
                return jsonify({"error": "Invalid Groq API key"}), 401
            return jsonify({"error": f"API request failed: {str(e)}"}), 500

    except Exception as e:
        print(f"Error in get_groq_models: {str(e)}")  # For debugging
        return jsonify({"error": str(e)}), 500
...
def get_gemini_models():
    try:
        ...
        print(f"Error in get_gemini_models: {str(e)}")  # For debugging
        return jsonify({"error": str(e)}), 500
...
def get_ollama_models():
    try:
        ...
        print(f"Error in get_ollama_models: {str(e)}")  # For debugging
        return jsonify({"error": str(e)}), 500
...
def save_settings():
    try:
        ...
        print(f"Error saving settings: {str(e)}")  # For debugging
        return jsonify({"error": str(e)}), 500
