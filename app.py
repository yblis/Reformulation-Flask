    ...
    try:
        ...
        try:
            ...
            filtered_models = [
                ...
                } for model in models if 'gemini' in model.
                name  # Filtrer les modèles contenant 'gemini' dans leur nom
            ]
...
def get_anthropic_models():
    try:
        # Charger la configuration des préférences
        preferences = reload_env_config()
        ...
        try:
            ...
            client = anthropic.Client(api_key=preferences.anthropic_api_key)
            ...
            filtered_models = [
                {
                    "id": model.id,
                    "name": model.id
                } for model in models if 'gpt' in
                model.id  # Filtrer les modèles contenant 'gpt' dans leur ID
            ]
...
def generate_email():
    try:
        ...
        # Initialize response based on provider
        response_text = None
        ...
        try:
            ...
            if not response_text:
                raise Exception(f"Failed to generate email with {provider}")
...
def translate():
    try:
        ...
        # Format prompt with user inputs
        formatted_prompt = preferences.translation_prompt.format(target_language=target_language)
        ...
        try:
            if provider == 'ollama':
                ...
                response = model.generate_content([
                    genai.types.Content(formatted_prompt, role="user"),
                    genai.types.Content(text, role="user")
                ])
                response_text = response.text

            if not response_text:
                raise Exception(f"Failed to translate with {provider}")
...
def reformulate():
    try:
        ...
        try:
            ...
            if not response_text:
                raise Exception(f"Failed to reformulate with {provider}")

            # Save to history
            history_entry = ReformulationHistory(
                original_text=text,
                context=context,
